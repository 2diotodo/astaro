...
function CommandLine(console) {
  ...
  this.set = function (config) {
    if (config.auth != null)
      this.console.find(".command-input *[data-auth]").text(config.auth);
    if (config.route != null)
      this.console.find(".command-input *[data-route]").text(config.route);
    if (config.input != null)
      this.console.find(".command-input *[data-input]").text(config.input);
  };
}
...
cli.on("^echo\\s[\"']?\\b(.+)\\b\\1?", function (output, matches) {
  ...
  return false;
  if (matches[2]) output.puts(matches[1]);
  else output.puts(matches[1].split(" ")[0]);
});

cli.on("^clears?$", function (output) {
  con.find(".command-line").remove();
  cli.consolize();
});

cli.on("^helps?$", function (output) {
  output.puts("Type:");
...
cli.on("^platforms?$", function (output) {
  output.puts("Browser Name: " + platform.name);
  output.puts("Browser Version: " + platform.version);
  output.puts("Browser Engine: " + platform.layout);
  output.puts("Browser Language: " + navigator.language);
  output.puts("Cookies Enabled: " + navigator.cookieEnabled);
  output.puts("Java Enabled: " + navigator.javaEnabled());
  output.puts("Operating System: " + platform.os);
  output.puts(
...
cli.on("^ips?$", function (output) {
  $(function () {
    $.getJSON("https://api.ipify.org?format=jsonp&callback=?", function (json) {
      output.puts("Current IP address is: " + json.ip);
    });
  });
});
...
cli.on("([a-zα-ω0-9-_]+)", function (output, match) {
  output.puts(match[1] + ": command not found");
});
...
com.on("keydown", function (e) {
  ...
  if (key == 13) {
    ...
    commands.forEach(function (command) {
      ...
      // Run Callback
      if (match && next)
        // if next == true keep processing commands
    ...
    var tmph = histDo.pop();
    if (tmph != undefined) histUp.push(tmph);
    com.val(tmph);
  }
});
...
/* https://raw.githubusercontent.com/bestiejs/platform.js/master/platform.js */
